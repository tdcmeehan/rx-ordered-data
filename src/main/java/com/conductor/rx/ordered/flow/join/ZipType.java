package com.conductor.rx.ordered.flow.join;

/**
 * Supported join types.  This is different from {@link JoinType} in that these do not represent full SQL joins, but
 * item-by-item joining of data.  Implicit is that keys are unique in each stream; if the keys are not unique, then the
 * behavior is undefined.  Use constructs that employ {@link JoinType} if you anticipate duplicate keys or choose not
